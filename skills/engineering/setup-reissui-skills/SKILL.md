...
description: Configure a repo for the reissui/skills engineering skills — confirm the GitHub remote and gh auth, agree the label set /plan and /ship use (the blocked label and any epic label), and record where design docs live. Run this once per repo before using /plan and /ship. Use when the user says /setup-reissui-skills or asks to set up these skills in a repo.
---

# /setup-reissui-skills — one-time per-repo setup

Run this once in a repo before using `/plan` and `/ship`. It confirms the
...
`/plan` and `/ship` use GitHub issues as the only state store. Verify, without
asking:
...
`/ship` parks a blocked issue with a label, and `/plan` may mark an epic. Confirm
...
- `epic` — optional, applied by `/plan` to the PRD epic issue.
...
`/plan` writes the PRD to a GitHub issue and nothing to the repo, but if the
...
in a few lines, and confirm `/plan` and `/ship` are ready to use. Persist
